module.exports = {
    ...
    execute(message, args) {
        if (!message.member.hasPermission("MOVE_MEMBERS"))
            return message.channel.send("**You Dont Have The Permissions To Move Users! - [MOVE_MEMBERS")
        
        let member = messgae.mentions.members.first() || messge.guild.members.cache.get(args[0]) || message.guild.members.cache.find(r => r.user.username.toLowerCase() === args[0].toLocaleLowerCase());

        if (!member)
            return message.channel.send("Unable to find the mentioned user in this guild.");
